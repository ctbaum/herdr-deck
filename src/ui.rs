...
fn draw_help(f: &mut Frame, area: Rect, app: &App, hits: &mut Vec<HitRegion>) {
    let palette = &app.palette;
    let rows = [
        ("mouse", "hover preview · click open · wheel move"),
        ("j/k · ↑/↓", "previous / next result"),
        ("^j/^k", "move while searching or choosing a checkout"),
        ("pgup/pgdn", "move ten results"),
        ("g/G · home/end", "first / last result"),
        ("h/l", "previous / next source"),
        ("1/2/3", "projects / sessions / cleanable"),
        ("^s/^g", "sessions / cleanable source"),
        ("/ · ^u", "search / clear search"),
        ("↵", "focus workspace · open item · accept or submit form"),
        ("tab/S-tab", "sessions: agent filter · forms: next field"),
        ("^o", "new cockpit for the selected local project"),
        ("alt+o", "toggle between the two most recent projects"),
